    ...
    void Update()
    {
        ...
        if (gameObject.CompareTag("Ground"))
        {
            if (transform.position.x <= -groundWidth)
            {
                ...
                // 2 ile çarpmamızın sebebi: 2 tane ground oluştuktan sonra yeni gelen ground diğerinin soluna geçmesi için(groundun boyutunun 2 katı kadar yer değiştirdi)
            }
        }
        ...
        else if (gameObject.CompareTag("Obstacle"))
        {
            ...
            {
                //Obstacle'in x konumu scriptin ekli oldugu gameobjectin konumundan kucukse yok et (Yani kameradan cikmissa)
                Destroy(gameObject);
            }
        }
